...
unexpr :: Type -> (Expr a1 (Expr a1 a2)) -> Expr a1 a2
unexpr _ e =
  case e of {
   Var _ v -> v;
   TT -> TT;
   AppIdent s d idc args -> AppIdent s d idc (unexpr s args);
   App s d f x -> App s d (unexpr (Arrow s d) f) (unexpr s x);
   Pair a b a0 b0 -> Pair a b (unexpr a a0) (unexpr b b0);
   Abs s d f -> Abs s d (\x -> unexpr d (f (Var s x)))}
...
betaFast _ =
  toFlat (Arrow (Type_primitive Unit) (List (Prod (Type_primitive Nat0)
    (Type_primitive Nat0))))
    (powf (\x _ ->
      beta1 (Arrow (Type_primitive Unit) (List (Prod (Type_primitive Nat0)
        (Type_primitive Nat0)))) x) (S (S (S (S (S (S (S (S (S (S (S (S (S (S
