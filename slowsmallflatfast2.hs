...
red'_ident :: Type -> Type -> Ident -> (Value a1) -> Value a1
red'_ident _ _ idc x =
  case idc of {
   Primitive0 _ v -> unsafeCoerce (Prelude.Right v);
   Let_In tx tC ->
    reflect tC (AppIdent (Prod tx (Arrow tx tC)) tC (Let_In tx tC)
      (reify (Prod tx (Arrow tx tC)) x));
   Nat_succ ->
    case unsafeCoerce x of {
     Prelude.Left _ ->
      reflect (Type_primitive Nat0) (AppIdent (Type_primitive Nat0)
        (Type_primitive Nat0) Nat_succ (reify (Type_primitive Nat0) x));
     Prelude.Right v -> unsafeCoerce (Prelude.Right (S v))};
   Nat_add ->
    reflect (Type_primitive Nat0) (AppIdent (Prod (Type_primitive Nat0)
      (Type_primitive Nat0)) (Type_primitive Nat0) Nat_add
      (reify (Prod (Type_primitive Nat0) (Type_primitive Nat0)) x));
   Nat_sub ->
    reflect (Type_primitive Nat0) (AppIdent (Prod (Type_primitive Nat0)
   ...
   Nil t ->
    reflect (List t) (AppIdent (Type_primitive Unit) (List t) (Nil t)
      (reify (Type_primitive Unit) x));
   Cons t ->
    reflect (List t) (AppIdent (Prod t (List t)) (List t) (Cons t)
      (reify (Prod t (List t)) x));
   Fst a b ->
    case unsafeCoerce x of {
     Prelude.Left _ ->
      reflect a (AppIdent (Prod a b) a (Fst a b) (reify (Prod a b) x));
     Prelude.Right p -> case p of {
                         (,) a0 _ -> a0}};
   Snd a b ->
    case unsafeCoerce x of {
     Prelude.Left _ ->
      reflect b (AppIdent (Prod a b) b (Snd a b) (reify (Prod a b) x));
     Prelude.Right p -> case p of {
                         (,) _ b0 -> b0}};
   Bool_rect t ->
   ...
   Z_opp ->
    reflect (Type_primitive Z1) (AppIdent (Type_primitive Z1) (Type_primitive
      Z1) Z_opp (reify (Type_primitive Z1) x));
   Z_div ->
    reflect (Type_primitive Z1) (AppIdent (Prod (Type_primitive Z1)
   ...
   Z_of_nat ->
    reflect (Type_primitive Z1) (AppIdent (Type_primitive Nat0)
      (Type_primitive Z1) Z_of_nat (reify (Type_primitive Nat0) x));
   Z_cast range ->
    reflect (Type_primitive Z1) (AppIdent (Type_primitive Z1) (Type_primitive
      Z1) (Z_cast range) (reify (Type_primitive Z1) x));
   Z_cast2 range ->
...
red' red'_ident0 _ e =
  case e of {
   ...
   AppIdent s d idc args -> red'_ident0 s d idc (red' red'_ident0 s args);
   App s d f x ->
    unsafeCoerce red' red'_ident0 (Arrow s d) f (red' red'_ident0 s x);
   Pair a b a0 b0 ->
    unsafeCoerce (Prelude.Right ((,) (red' red'_ident0 a a0)
      (red' red'_ident0 b b0)));
   Abs _ d f -> unsafeCoerce (\v -> red' red'_ident0 d (f v))}

red :: Type -> (Expr0 Ident) -> Expr Ident a1
red t e =
  reify t (red' red'_ident t (e __))
...
toFlatFFromFlat_Fast2 :: () -> Expr1
toFlatFFromFlat_Fast2 _ =
  toFlat (Arrow (Type_primitive Unit) (Type_primitive Z1)) (\_ ->
    red (Arrow (Type_primitive Unit) (Type_primitive Z1)) (\_ ->
