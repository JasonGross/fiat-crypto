...
FAST_TARGETS += archclean clean cleanall clean-coqprime clean-bbv printenv clean-old update-_CoqProject regenerate-curves Makefile.coq
...
update-_CoqProject::
	...
	$(HIDE)(echo '-R $(SRC_DIR) $(MOD_NAME)'; (git ls-files 'src/*.v' | $(SORT_COQPROJECT))) > _CoqProject

$(VOFILES): | coqprime bbv
...
specific: $(SPECIFIC_VO) coqprime bbv
non-specific: $(NON_SPECIFIC_VO) coqprime bbv
coq: $(COQ_VOFILES) coqprime bbv
lite: $(LITE_VOFILES) coqprime bbv
only-heavy: $(HEAVY_VOFILES) coqprime bbv
curves-proofs: $(CURVES_PROOFS_VOFILES) coqprime bbv
no-curves-proofs: $(NO_CURVES_PROOFS_VOFILES) coqprime bbv
no-curves-proofs-non-specific: $(NO_CURVES_PROOFS_NON_SPECIFIC_VOFILES) coqprime bbv
specific-display: $(SPECIFIC_DISPLAY_VO:.vo=.log) coqprime bbv
specific-c: $(filter-out $(UNMADE_C_FILES),$(SPECIFIC_DISPLAY_VO:Display.vo=.c) $(SPECIFIC_DISPLAY_VO:Display.vo=.h)) coqprime bbv
selected-specific: $(SELECTED_SPECIFIC_VOFILES) coqprime bbv
selected-specific-display: $(SELECTED_SPECIFIC_DISPLAY_VO:.vo=.log) coqprime bbv
selected-c: $(filter-out $(UNMADE_C_FILES),$(SELECTED_SPECIFIC_DISPLAY_VO:Display.vo=.c) $(SELECTED_SPECIFIC_DISPLAY_VO:Display.vo=.h)) coqprime bbv
nonautogenerated-specific: $(NONAUTOGENERATED_SPECIFIC_VOFILES) coqprime bbv
nonautogenerated-specific-display: $(NONAUTOGENERATED_SPECIFIC_DISPLAY_VO:.vo=.log) coqprime bbv
nonautogenerated-c: $(filter-out $(UNMADE_C_FILES),$(NONAUTOGENERATED_SPECIFIC_DISPLAY_VO:Display.vo=.c) $(NONAUTOGENERATED_SPECIFIC_DISPLAY_VO:Display.vo=.h)) coqprime bbv
display: $(DISPLAY_VO:.vo=.log) coqprime bbv
...
COQPATH?=${CURDIR}/$(COQPRIME_FOLDER):${CURDIR}/$(BBV_FOLDER)
export COQPATH
...
bbv:
	$(MAKE) --no-print-directory -C $(BBV_FOLDER)

clean-bbv:
	$(MAKE) --no-print-directory -C $(BBV_FOLDER) clean

install-bbv:
	$(MAKE) --no-print-directory -C $(BBV_FOLDER) install
...
cleanall:: clean clean-coqprime clean-bbv

install: coq install-coqprime install-bbv
