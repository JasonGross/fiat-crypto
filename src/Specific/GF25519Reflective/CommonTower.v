...
Definition uninterp_of_towerW {T} count_in count_out
  : tower_nd T (interp_flat_type WordW.interp_base_type (Expr_n_OpT count_out)) count_in
    -> tower_nd T (Tuple.tuple fe25519W count_out) count_in
  := impl_under_tower_nd _ _ _ (flat_interp_tuple (n:=count_out)) _.
...
Definition interp_nm_expr''' count_in count_out (e : Expr_nm_Op count_in count_out)
  : Tower.tower_nd Specific.GF25519BoundedCommon.fe25519W
                   ...
                   count_in.
Proof.
  generalize (interp_nm_expr'' count_in count_out e).
  apply uninterp_of_towerW.
Defined.
Definition interp_nm_expr count_in count_out (e : Expr_nm_Op count_in count_out)
  : Tower.tower_nd Specific.GF25519BoundedCommon.fe25519W
                   _
                   count_in
  := Eval cbv [interp_nm_expr''' interp_nm_expr'' interp_nm_expr' ApplyInterpedAll' curry_unop_fe25519W Expr_nm_OpT fst snd SmartArrow fe25519T] in
...
Notation rexpr_sig T uncurried_op :=
  { rexprZ
...
Ltac make_bounds_prop bounds orig_bounds :=
...
Lemma Expr_nm_Op_correct_and_bounded
      ...
      (H0 : forall xy Hxy,
          let args := nm_op_args_to_bounded pred_count_in (S pred_count_out) xy Hxy in
          match LiftOption.of'
      ...
      (H1 : forall xy Hxy,
          ...
          with
          | Some bounds => nm_op_bounds_good (S pred_count_in) pred_count_out bounds = true
          | None => False
          end)
  : nm_op_correct_and_bounded (S pred_count_in) (S pred_count_out) (MapInterp (fun _ x => x) ropW) op.
Proof.
  apply inm_op_correct_and_bounded_iff_prefix.
  change inm_op_correct_and_bounded_prefix with inm_op_correct_and_bounded_prefix'.
     ...
     *) admit. }
  {
    ...
    Compute (@Tuple.map 1).
    change (@Tuple.map 1) with (fun A B (f : A -> B) xs => f xs).
  move op at bottom.

  unfold inm_op_correct_and_bounded.
