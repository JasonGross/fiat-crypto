...
Notation correct_and_bounded_genT ropW'v ropZ_sigv
  := (let ropW' := ropW'v in
      ...
      /\ interp_type_rel_pointwise Relations.related_bounds (Interp (@BoundedWordW.interp_op) ropW') (Interp (@Bounds.interp_op) ropW')
...
Local Ltac make_bounds_prop bounds orig_bounds :=
  ...
  repeat (refine (_ &&
                    match snd bounds' with
                    | Some bounds' => let (l, u) := snd bounds in
                                      let (l', u') := bounds' in
                                      ((l' <=? l) && (u <=? u'))%Z%bool
                    | None => false
                    end)%bool;
...
(*
Definition unop_bounds_good (bounds : interp_flat_type (fun _ => Bounds.bounds) (codomain ExprUnOpT)) : bool.
Proof. make_bounds_prop bounds ExprUnOp_bounds. Defined.
Definition binop_bounds_good (bounds : interp_flat_type (fun _ => Bounds.bounds) (codomain ExprBinOpT)) : bool.
Proof. make_bounds_prop bounds ExprUnOp_bounds. Defined.
Definition unopFEToWire_bounds_good (bounds : interp_flat_type (fun _ => Bounds.bounds) (codomain ExprUnOpFEToWireT)) : bool.
Proof. make_bounds_prop bounds ExprUnOpWireToFE_bounds. Defined.
Definition unopWireToFE_bounds_good (bounds : interp_flat_type (fun _ => Bounds.bounds) (codomain ExprUnOpWireToFET)) : bool.
Proof. make_bounds_prop bounds ExprUnOp_bounds. Defined.
(* TODO FIXME(jgross?, andreser?): Is every function returning a single Z a boolean function? *)
Definition unopFEToZ_bounds_good (bounds : interp_flat_type (fun _ => Bounds.bounds) (codomain ExprUnOpFEToZT)) : bool.
Proof.
  refine (let (l, u) := bounds in ((0 <=? l) && (u <=? 1))%Z%bool).
Defined.
Definition op9_4_bounds_good (bounds : interp_flat_type (fun _ => Bounds.bounds) (codomain Expr9_4OpT)) : bool.
...
Notation compute_bounds rexpr rexpr_bounds
  := (@Bounds.ComputeBounds _ rexpr rexpr_bounds)
       (only parsing).
