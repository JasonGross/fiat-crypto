...
Definition raddZ_sig
  : { raddZ
    | interp_type_gen_rel_pointwise (fun _ => Logic.eq) (Interp (@Z.interp_op) (t:=ExprBinOpT) raddZ) (uncurry_binop_fe25519 carry_add) }.
Proof.
  cbv beta iota delta [uncurry_binop_fe25519 carry_add interp_type_rel_pointwise interp_type_gen_rel_pointwise ExprBinOpT interp_flat_type Z.interp_base_type interp_base_type].
  eexists; intros.
  Reify_rhs.
  Focus 2.
  ...
  lazymatch goal with
  | [ |- context G[Interp ?iop ?expr] ]
    => pose expr as v;
         let G' := context G[Interp iop v] in
         change G'
  end;
    vm_compute in v;
    assert (Hwf : Wf v) by reflect_Wf base_type_eq_semidec_is_dec op_beq_bl.

  Require Import InterpWf.
  SearchAbout wf interp.

  let v := match goal with
  SearchAbout Interp Linearize.Linearize.
...
Local Ltac reify_correct_and_bounded :=
  eexists; intros; split; cbv [fe25519W] in *.
Definition radd_sig
  : { radd : ExprBinOp
    | binop_correct_and_bounded radd carry_add }.
Proof.
  reify_correct_and_bounded.
