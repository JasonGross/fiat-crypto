...
Section with_curves.
  ...
  Record montgomery_side_conditions :=
    {
      ...
      vm_add_patterned := (fun ab => add_patterned r sz (m_enc, ab));
      vm_sub_patterned := (fun ab => sub_patterned r sz (m_enc, ab));
      vm_opp_patterned := (fun a => opp_patterned r sz (m_enc, a));
      vm_nonzero_patterned := (nonzero_patterned sz);
      vm_mul_patterned := (fun ab => redc_patterned r sz sz m'' (m_enc, ab));

      vm_compiled_preadd : vm_compute_evar_package_vm_large (Compiler.fill_first_of_pair_tupleZ (compiled_preadd r sz) m_enc);
      vm_compiled_presub : vm_compute_evar_package_vm_large (Compiler.fill_first_of_pair_tupleZ (compiled_presub r sz) m_enc);
      vm_compiled_preopp : vm_compute_evar_package_vm_large (Compiler.fill_first_of_pair_tupleZ (compiled_preopp r sz) m_enc);
      vm_compiled_prenonzero : vm_compute_evar_package_vm_large (compiled_prenonzero sz);
      vm_compiled_premul : vm_compute_evar_package_vm_large (Compiler.fill_first_of_pair_tupleZ (compiled_preredc' r sz sz m'') m_enc);
      ...
      : admit_package
          (forall x y, evalE (Interp (val addZ) (x, y))
                       = Compiler.do_apply (vm_add_patterned (evalE x, evalE y)))
      := admit;
      interp_subZ
      : admit_package
          (forall x y, evalE (Interp (val subZ) (x, y))
                       = Compiler.do_apply (vm_sub_patterned (evalE x, evalE y)))
      := admit;
      interp_oppZ
      : admit_package
          (forall x, evalE (Interp (val oppZ) x)
                     = Compiler.do_apply (vm_opp_patterned (evalE x)))
      := admit;
      interp_mulZ
      : admit_package
          (forall x y, evalE (Interp (val mulZ) (x, y))
                       = Compiler.do_apply (vm_mul_patterned (evalE x, evalE y)))
      := admit;
      interp_nonzeroZ
      : admit_package
          (forall x, Interp (val nonzeroZ) x
                     = Compiler.do_apply (vm_nonzero_patterned (evalE x)))
      := admit;
  ...
  Local Ltac eval_t_whenever :=
    repeat first [ progress cbn in *
                 ...
                 | rewrite redc_patterned_correct
                 | rewrite opp_patterned_correct
                 | rewrite nonzero_patterned_correct
                 ...
                 | rewrite (evar_package_pf_eq curve_scd.(vm_nonzero_patterned))
                 (*| rewrite (evar_package_pf_eq curve_scd.(vm_freeze_patterned))*)
                 | rewrite interp_addZ
                 | rewrite interp_subZ
                 | rewrite interp_oppZ
                 | rewrite interp_mulZ
                 | rewrite interp_nonzeroZ
  ...
  Proof.
    ...
    eval_t.
    destruct_head'_and; repeat apply conj; intros; eauto.
  Qed.
  ...
  Proof.
    ...
    { abstract (cbv [eval']; setoid_rewrite flat_interp_tuple_untuple; rewrite eval_zeroZ; reflexivity). }
    { abstract (cbv [eval']; setoid_rewrite flat_interp_tuple_untuple; rewrite eval_oneZ; reflexivity). }
