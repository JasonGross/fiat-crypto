...
Section gen.
  ...
  Record solinas_side_conditions :=
    {
      ...
      vm_compiled_preadd : vm_compute_evar_package (compiled_preadd wt sz);
      vm_compiled_presub : vm_compute_evar_package (Compiler.fill_first_of_pair_tupleZ (compiled_presub wt sz) coef);
      vm_compiled_premul : vm_compute_evar_package (compiled_premul wt sz sz2);
      vm_compiled_preopp : vm_compute_evar_package (Compiler.fill_first_of_pair_tupleZ (compiled_preopp wt sz) coef);
      vm_compiled_prereduce : vm_compute_evar_package (compiled_prereduce wt sz2 sz s c);
      vm_compiled_prereduce_sz_sz : vm_compute_evar_package (compiled_prereduce wt sz sz s c);
      vm_compiled_prereduce_sz_1p5 : vm_compute_evar_package (compiled_prereduce wt (sz + half_sz) sz s c);
      vm_compiled_prechained_carries_reduce : vm_compute_evar_package (compiled_prechained_carries_reduce wt sz s c carry_chains);
      vm_compiled_prekaratsuba_mul
      : vm_compute_evar_package (if curve.(karatsuba)
                                 then Some (compiled_prekaratsuba_mul wt half_sz sz sqrt_s)
                                 else None);
      vm_compiled_pregoldilocks_mul
      : vm_compute_evar_package (if curve.(goldilocks)
                                 then Some (compiled_pregoldilocks_mul wt half_sz sz sqrt_s)
                                 else None);
      vm_compiled_prefreeze
      : vm_compute_evar_package (if curve.(freeze)
      ...
      := ((val premulZ) ∘ (fun var => Abs (fun x => (SmartMap.SmartVarf x, SmartMap.SmartVarf x)%expr)))%expr;
      ...
      mul_code_correct
      : match mul_code_casted with
        | None => True
        | Some v
          => eq_by_Zring_prod_package
               (forall ab,
                   v ab
                   = Interp (val reduceZ) (Interp (val premulZ) ab))
        end;
      square_code_correct
      : match square_code_casted with
        | None => True
        | Some v
          => eq_by_Zring_prod_package
               (forall a,
                   v a
                   = Interp (val reduceZ) (Interp (val_presquareZ) a))
        end;

      interp_addZ
      : eq_by_Zring_prod_package
          (forall x y, evalE (Interp (val addZ) (x, y))
                       = Compiler.do_apply (add_patterned wt sz (evalE x, evalE y)))
      := admit;
      interp_subZ
      : eq_by_Zring_prod_package
          (forall x y, evalE (Interp (val subZ) (x, y))
                       = Compiler.do_apply (sub_patterned wt sz (coef, (evalE x, evalE y))))
      := admit;
      interp_oppZ
      : eq_by_Zring_prod_package
          (forall x, evalE (Interp (val oppZ) x)
                     = Compiler.do_apply (opp_patterned wt sz (coef, (evalE x))))
      := admit;
      interp_carryZ
      : admit_package
          (eq_by_Zring_prod_package
             (forall x, evalE (Interp (val carryZ) x)
                        = Compiler.do_apply (chained_carries_reduce_patterned wt sz s c carry_chains (evalE x))))
      := admit;
      interp_reduceZ
      : admit_package
          (eq_by_Zring_prod_package
             (forall x, evalE (Interp (val reduceZ) x)
                        = Compiler.do_apply (reduce_patterned wt sz2 sz s c (evalE2 x))))
      := admit;
      interp_reduceZ_sz_sz
      : admit_package
          (eq_by_Zring_prod_package
             (forall x, evalE (Interp (val reduceZ_sz_sz) x)
                        = Compiler.do_apply (reduce_patterned wt sz sz s c (evalE x))))
      := admit;
      interp_reduceZ_sz_1p5
      : admit_package
          (eq_by_Zring_prod_package
             (forall x, evalE (Interp (val reduceZ_sz_1p5) x)
                        = Compiler.do_apply (reduce_patterned wt (sz + half_sz) sz s c (evalE1p5 x))))
      := admit;
      interp_premulZ
      : admit_package
          (forall x y, evalE2 (Interp (val premulZ) (x, y))
                    = Compiler.do_apply (mul_patterned wt sz sz2 (evalE x, evalE y)))
      := admit;
      ...
      : admit_package
          (forall x y, option_map
                         ...
                         (val prekaratsuba_mulZ)
                       = (if curve.(karatsuba)
                          then Some (Compiler.do_apply
                                       (karatsuba_mul_patterned wt half_sz sz sqrt_s (evalE x, evalE y)))
                          else None))
      := admit;
      ...
      : admit_package
          (forall x y, option_map
                       ...
                       = (if curve.(goldilocks)
                          then Some (Compiler.do_apply
                                       (goldilocks_mul_patterned wt half_sz sz sqrt_s (evalE x, evalE y)))
                          else None))
      := admit;
      interp_freezeZ
      : admit_package
          (forall x, option_map
                     ...
                     = (if curve.(freeze)
                        then Some (Compiler.do_apply
                                     (freeze_patterned wt sz (Z.ones bitwidth) (m_enc, (evalE x))))
                        else None))
      := admit;
  ...
  Local Ltac eval_t_whenever :=
    repeat first [ progress cbn -[Interp] in *
                 | progress intros
                 | progress cbv [eval' evalZ' eval2' evalZ2' eval1p5' evalZ1p5' mul_code_casted square_code_casted LetIn.Let_In val_presquareZ] in *
                 | progress subst
                 ...
                 | match goal with
                   ...
                   | [ |- context[Interp (val (carryZ _))] ] => rewrite interp_carryZ
                   | [ |- context[Interp (val (reduceZ _))] ] => rewrite interp_reduceZ
                   | [ |- context[Interp (val (reduceZ_sz_sz _))] ] => rewrite interp_reduceZ_sz_sz
                   | [ |- context[Interp (val (reduceZ_sz_1p5 _))] ] => rewrite interp_reduceZ_sz_1p5
                   | [ |- context[Interp (val (premulZ _))] ] => rewrite interp_premulZ
                   | [ |- context[option_map _ (val (prekaratsuba_mulZ _))] ] => rewrite interp_prekaratsuba_mulZ
                   | [ |- context[option_map _ (val (pregoldilocks_mulZ _))] ] => rewrite interp_pregoldilocks_mulZ
                   | [ |- context[option_map _ (val (freezeZ _))] ] => rewrite interp_freezeZ
                   | [ |- context[add_patterned] ] => rewrite add_patterned_correct
                   | [ |- context[sub_patterned] ] => rewrite sub_patterned_correct
                   | [ |- context[mul_patterned] ] => rewrite mul_patterned_correct
                   | [ |- context[opp_patterned] ] => rewrite opp_patterned_correct
                   | [ |- context[reduce_patterned] ] => rewrite reduce_patterned_correct
                   | [ |- context[chained_carries_reduce_patterned] ] => rewrite chained_carries_reduce_patterned_correct
                   | [ |- context[karatsuba_mul_patterned] ] => rewrite karatsuba_mul_patterned_correct
                   | [ |- context[goldilocks_mul_patterned] ] => rewrite goldilocks_mul_patterned_correct
                   | [ |- context[freeze_patterned] ] => rewrite freeze_patterned_correct
                   | [ |- context[Interp (_ ∘ _)%expr] ] => rewrite InterpCompose
                   | [ |- context G[Interp (fun var => @?f var)] ]
                     => let G' := context G[interp interp_op (f _)] in
                        change G'
                   | [ |- context[interpf _ (SmartMap.SmartVarf _)] ] => rewrite !SmartMap.interpf_SmartVarf
                   ...
                   | [ |- ?x = ?x ] => reflexivity
                   | [ H : context[orb _ true] |- _ ] => rewrite Bool.orb_true_r in H
                   | [ H : context[Interp ?e _ = _] |- context[Interp ?e] ] => rewrite H
                   | [ H : context[?e _ = _] |- context[?e] ] => rewrite H
                   end
  ...
  Proof.
    cbn; intro a; setoid_rewrite <- (eval_premulZ (a, a)); cbn; cbv [Let_In].
    cbv [Interp]; rewrite <- interpf_invert_Abs.
    rewrite !SmartMap.interpf_SmartVarf.
    reflexivity.
  Qed.
  ...
  Lemma eval_prekaratsuba_mulZ
    : forall ab,
      option_map (fun prekaratsuba_mulZ : Expr (exprT sz 2 sz)
                 ...
                 prekaratsuba_mulZ'
      = if curve.(karatsuba)
        then Some ((eval' (fst ab) * eval' (snd ab))%F)
        else None.
  ...
  Proof.
    ...
    { pose proof (curve_scd.(mul_code_correct)).
      ...
      destruct curve_scd.(mul_code_casted); cbv [eq_by_Zring_prod_package] in *;
        eval_t. }
    { cbn -[Interp Compose] in *.
      ...
      destruct curve_scd.(pregoldilocks_mulZ) as [ [?|] ?];
        [ destruct curve.(goldilocks) | destruct curve_scd.(prekaratsuba_mulZ) as [ [?|] ?]; [ destruct curve.(karatsuba) | ] ];
  ...
  Proof.
    ...
    { pose proof (curve_scd.(square_code_correct)).
      cbn [val]; intros; rewrite InterpCompose, eval_reduceZ_sz_sz.
      destruct curve_scd.(square_code_casted); cbv [eq_by_Zring_prod_package] in *;
        eval_t. }
    { cbn -[Interp Compose] in *.
  ...
  Proof.
    intro; cbv [carry_addZ'].
    rewrite InterpCompose, eval_carryZ, eval_addZ; reflexivity.
  Qed.
  ...
  Proof.
    intro; cbv [carry_subZ' LetIn.Let_In].
    rewrite InterpCompose, eval_carryZ, eval_subZ; reflexivity.
  Qed.
  ...
  Proof.
    intro; cbv [carry_oppZ'].
    rewrite InterpCompose, eval_carryZ, eval_oppZ; reflexivity.
  Qed.
