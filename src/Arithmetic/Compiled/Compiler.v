...
Definition IdTupleWithAltZOrExpr_cps {var n R} (T:=TZ)
  ...
  : @ZOrExpr var R
  := match invert_Pairs (x _ of_tuple_var), invert_Pairs (y _ of_tuple_var) with
     | Some x, Some y
     ...
     | None, None
       => inExpr
            (LetIn
               (OpZOrExpr IdWithAlt (PairZOrExpr (x _ of_tuple_var)
                                                 (y _ of_tuple_var)))
               (fun v => f (Tuple.map (fun v => inExpr (Var v))
                                      (flat_interp_tuple (n:=n) v))))
     end.
...
Ltac do_pattern_strip_replace_with SmartVarVarf var T t
  ...
  let t :=
      constr:(
        ...
        t') in
  (** N.B. If we don't βδ-reduce [BoolCaseZOrExpr] before running
           [vm_compute], [vm_compute] is extremely slow *)
  let t := (eval cbv beta zeta delta [BoolCaseZOrExpr] in t) in
  t.
...
Ltac do_pattern_strip_replace_id_with_alt SmartVarVarf var t :=
  ...
  | context[@CPSUtil.id_tuple_with_alt_cps (ZOrExpr ?var ?T) (ZOrExpr ?var (Tbase ?tz)) ?n]
    => let t := (eval pattern (@CPSUtil.id_tuple_with_alt_cps (ZOrExpr var T) (ZOrExpr var (Tbase tz)) n) in t) in
       ...
       let t := (eval cbv [IdTupleWithAltZOrExpr_cps] in
                    (t (@IdTupleWithAltZOrExpr_cps var n T))) in
       rec_call t
  | _ => t
  end.
...
Ltac do_compile_sig op_cps appf :=
  ...
  let check_tuple tuple :=
      let dummy := match constr:(Set) with
                   | _ => unify tuple (Tuple.tuple Z)
                   | _ => let T := (eval cbv beta zeta in t_ty) in
                          idtac "Warning: do_compile_sig: Unsupported type" T
                   end in
      constr:(I) in
  let t := lazymatch (eval cbv beta zeta in t_ty) with
           | forall (n : nat) (R : Type) (f : ?tupleZ (@?n_expr n) -> R), _
             => let test := check_tuple tupleZ in
                compile varf SmartVarVarf (fun var n => op_cps n (@ZOrExpr (varf var) (tuple tZ (n_expr n))))
           | forall (n1 n2 : nat) (R : Type) (f : ?tupleZ (@?n_expr n1 n2) -> R), _
             => let test := check_tuple tupleZ in
                compile varf SmartVarVarf (fun var n1 n2 => op_cps n1 n2 (@ZOrExpr (varf var) (tuple tZ (n_expr n1 n2))))
           | forall (n : nat) (R : Type) (m : nat) (f : ?tupleZ (@?n_expr n) -> R), _
             => let test := check_tuple tupleZ in
                compile varf SmartVarVarf (fun var n => op_cps n (@ZOrExpr (varf var) (tuple tZ (n_expr n))))
           | ?T
...
Declare Reduction compiler_preprered
  := cbv [id
            ...
            BoolCaseZOrExpr
            IdWithAltZOrExpr
            IdTupleWithAltZOrExpr
            IdTupleWithAltZOrExpr_cps
            MulSplitZOrExpr
            AddWithGetCarryZOrExpr
            SubWithGetBorrowZOrExpr*)
            (*inZ
            of_tupleZ
            of_tuple_var
            failb
...
Goal True.
  pose compiled_prekaratsuba_mul' as e.
  cbv beta in e.
  cbv [compiled_prekaratsuba_mul'] in e.
  Set Printing Depth 1000000.
  Require Import LetIn.
  Import Compilers.Syntax.
  Import ZExtended.Syntax.
  Import Z.Syntax.
  Notation E := (Syntax.Expr _).
  cbv [id
         ZExtended.InlineConstAndOp.InlineConstAndOp Compilers.InlineConstAndOp.InlineConstAndOp Inline.InlineConstGen Inline.inline_const_gen
         InlineConstAndOp.Const
         Linearize.Linearize Linearize.Linearize_gen Linearize.linearize_gen
         Eta.ExprEta Eta.expr_eta
         ZExtended.MapBaseType.MapBaseType Compilers.MapBaseType.MapBaseType Compilers.MapBaseType.MapBaseType' MapBaseType.map_base_type
         fst snd
         SmartMap.SmartVarfMap SmartMap.SmartVarVarf
         ZOrExprSmartPairf inZ BoolCaseZOrExpr MulSplitZOrExpr AddWithGetCarryZOrExpr SubWithGetBorrowZOrExpr
         Util.make_const cast_const
      ] in e.
  ...
  Locate place_cps.
  Print Core.B.Positional.place_cps.
...
Inductive hold {T} (v : T) := do_hold.

Ltac do_pattern_strip t :=
  let t := (eval cbv beta zeta in
               ltac:(let t' := fresh "t" in
                     pose proof (do_hold t) as t';
                     repeat autorewrite with pattern_runtime in t';
                     let t := lazymatch type of t' with hold ?t => t end in
                     exact t)) in
...
Notation do_pattern_strip t := ltac:(do_pattern_strip t) (only parsing).
Section do_apply.
  Let do_apply_sig : { T : Type & T }.
  Proof.
    eexists.
    refine (fun R => _).
    let f' := constr:(fun (T : Type) (k : T) => ltac:(let v := pattern_strip k in exact v)) in
    let T := lazymatch type of (f' R) with _ -> ?T => T end in
    refine (fun f : T => _).
    let t := apply_patterned f in
    exact t.
  Defined.

  Definition do_apply {R} f := Eval cbv [projT1 projT2 do_apply_sig] in projT2 do_apply_sig R f.
End do_apply.

Definition compile_const {A}
           (v : interp_flat_type Z.Syntax.interp_base_type A)
  : Z.Syntax.Expr
      (Unit -> A)
  := fun var
     => Abs (fun _
             => SmartPairf
                  (SmartVarfMap
                     (fun t v => Op (Z.Syntax.Util.make_const t v) TT)
                     v)).

Definition fill_first_of_pair {A B C}
           (f : option
                  (Z.Syntax.Expr
                     (lift_flat_type Util.unextend_base_type (A * B)
                      -> C)))
           (v : interp_flat_type Z.Syntax.interp_base_type (lift_flat_type Util.unextend_base_type A))
  : option
      (Z.Syntax.Expr
         (lift_flat_type Util.unextend_base_type B
          -> C))
  := option_map
       (fun f =>
          ExprEta
            (Z.Inline.InlineConst
               (fun var
                => Abs (fun b : interp_flat_type _ (lift_flat_type _ B)
                        => LetIn
                             (invert_Abs (compile_const v _) tt)
                             (fun a : interp_flat_type _ (lift_flat_type _ A)
                              => invert_Abs
                                   (T:=Arrow (Prod _ _) _)
                                   (f var)
                                   (a, b))))))
...
Local Notation pretransfer_const n v
  := (flat_interp_untuple (n:=n) (T:=tZ) v).
Local Notation pretransfer_const' n v
  := (flat_interp_untuple (n:=n) (T:=Tbase Z.Syntax.TZ) v).
Local Notation transfer_const n v
  := (transfer_interp_flat_type
        (var1 := fun t => Z.Syntax.interp_base_type (Util.unextend_base_type t))
        Util.unextend_base_type
        (fun t v => v)
        (pretransfer_const n v)).

Definition compile_tupleZ {n}
           (v : Tuple.tuple Z n)
  : Z.Syntax.Expr
      (Unit -> Syntax.Tbase Z.Syntax.TZ^n)
  := compile_const (pretransfer_const' n v).

Definition fill_first_of_pair_tupleZ {n B C}
           (f : option
                  (Z.Syntax.Expr
                     (lift_flat_type Util.unextend_base_type (tZ^n * B)
                      -> C)))
           (v : Tuple.tuple Z n)
  : option
      (Z.Syntax.Expr
