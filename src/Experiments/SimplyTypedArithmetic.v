      ...
      Section gen.
        ...
        Definition gen_pinterp {t} (idc : pident t) : type.interp base.interp (parametric.subst t)
          := match idc in pident t return type.interp base.interp (parametric.subst t) with
             ...
             | Z_add_get_carry_concrete s => Z.add_get_carry s
             | Z_add_with_carry => Z.add_with_carry
             | Z_add_with_get_carry => Z.add_with_get_carry_full
             | Z_add_with_get_carry_concrete s => Z.add_with_get_carry s
             | Z_sub_get_borrow => Z.sub_get_borrow_full
             | Z_sub_get_borrow_concrete s => Z.sub_get_borrow s
             | Z_sub_with_get_borrow => Z.sub_with_get_borrow_full
             | Z_sub_with_get_borrow_concrete s => Z.sub_with_get_borrow s
             | Z_zselect => Z.zselect
...
*)
  ...
  Notation "'ADD_128' ( x ,  y )" := (#(ident.Z_cast2 (uint128, bool)%core) @ (ident.Z.add_get_carry_concrete TwoPow256 @@ (x, y)))%expr : expr_scope.
  Notation "'ADDC_256' ( x ,  y ,  z )" := (#(ident.Z_cast2 (uint256, bool)%core @@ (ident.Z.add_with_get_carry_concrete TwoPow256 @@ (x, y, z)))%expr : expr_scope.
  Notation "'ADDC_128' ( x ,  y ,  z )" := (#(ident.Z_cast2 (uint128, bool)%core @@ (ident.Z.add_with_get_carry_concrete TwoPow256 @@ (x, y, z)))%expr : expr_scope.
  Notation "'SUB_256' ( x ,  y )" := (#(ident.Z_cast2 (uint256, bool)%core @@ (ident.Z.sub_get_borrow_concrete TwoPow256 @@ (x, y)))%expr : expr_scope.
  Notation "'ADDM' ( x ,  y ,  z )" := (#(ident.Z_cast uint256 @@ (ident.Z.add_modulo @@ (x, y, z)))%expr : expr_scope.
  Notation "'RSHI' ( x ,  y ,  z )" := (#(ident.Z_cast _ @@ (ident.Z.rshi_concrete _ z @@ (x, y)))%expr : expr_scope.
  Notation "'SELC' ( x ,  y ,  z )" := (#(ident.Z_cast uint256 @@ (ident.Z.zselect @@ (x, y, z)))%expr : expr_scope.
  Notation "'SELM' ( x ,  y ,  z )" := (#(ident.Z_cast uint256 @@ (ident.Z.zselect @@ (Z.cast bool @@ (Z.cc_m_concrete _ @@ x), y, z)))%expr : expr_scope.
  Notation "'SELL' ( x ,  y ,  z )" := (#(ident.Z_cast uint256 @@ (ident.Z.zselect @@ (Z.cast bool @@ (Z.land 1 @@ x), y, z)))%expr : expr_scope.
*)
