    ...
    Module C.
      ...
      Fixpoint make_return_assignment_of_arith {t}
        ...
        := match t return var_data t -> default.Notations.expr t -> _ with
           ...
           | type.list Compilers.type.Z
             => fun '(n, r, len) e
                => (ls <- arith_expr_of_PHOAS e (Some (repeat r len));
                      List.fold_right
                        (fun a b
                         => match b with
...
Module Pipeline.
  ...
  Section show.
    ...
    Global Instance show_pair {A B} `{Show A, Show B} : Show (A * B)
      := fun _ '(a, b) => "(" ++ show true a ++ ", " ++ show true b ++ ")".
    Definition show_prim_zrange_opt_interp {t}
      : Show (ZRange.type.primitive.option.interp t)
      := match t return Show (ZRange.type.primitive.option.interp t) with
         | type.unit => _
         | type.Z => _
         | type.nat => _
         | type.bool => _
    ...
    Fixpoint show_zrange_opt_interp {t} : Show (ZRange.type.option.interp t)
      := fun parens
         => match t return ZRange.type.option.interp t -> string with
            | type.type_primitive t
              => fun v : ZRange.type.primitive.option.interp t
                 => show parens v
            | type.prod A B
              => fun '(a, b)
                 => "(" ++ @show_zrange_opt_interp A true a
                        ++ ", " ++ @show_zrange_opt_interp B true b
                        ++ ")"
    ...
    Global Instance show_primitive_type : Show type.primitive
      := fun p t
         => match t with
            | type.unit => "()"
            | type.Z => "ℤ"
    ...
    Fixpoint show_type (p : bool) (t : type) : string
      := match t with
         | type.type_primitive x => show p x
         | type.prod A B => maybe_wrap_parens p (show_type true A ++ " * " ++ show_type true B)
         | type.arrow s d => maybe_wrap_parens p (show_type true s ++ " → " ++ show_type true d)
         | type.list A => "list " ++ show_type true A
         end.
    Global Instance: Show type := show_type.

    Local Notation NewLine := (String "010" "") (only parsing).

    Fixpoint find_too_loose_bounds {t}
      : ZRange.type.option.interp t -> ZRange.type.option.interp t-> bool * list (nat * nat) * list (zrange * zrange)
      := match t return ZRange.type.option.interp t -> ZRange.type.option.interp t-> bool * list (nat * nat) * list (zrange * zrange) with
         | type.unit
         | type.nat
         | type.bool
         ...
         | type.prod A B
           => fun '(ra, rb) '(ra', rb')
              => let '(b1, lens1, ls1) := @find_too_loose_bounds A ra ra' in
                 let '(b2, lens2, ls2) := @find_too_loose_bounds B rb rb' in
                 ...
                 | Some ls1, Some ls2
                   => List.fold_right
                        ...
                        (false,
                         ...
                         nil)
                        (List.map
                           (fun '(a, b) => @find_too_loose_bounds A a b)
    ...
    Global Instance show_ErrorMessage : Show ErrorMessage
      := fun parens e
         => maybe_wrap_parens
              parens
              match e with
              | Computed_bounds_are_not_tight_enough t computed_bounds expected_bounds d syntax_tree arg_bounds
                => ("Computed bounds " ++ show true computed_bounds ++ " are not tight enough (expected bounds not looser than " ++ show true expected_bounds ++ ")." ++ NewLine)
                     ++ (explain_too_loose_bounds computed_bounds expected_bounds ++ NewLine)
...
Section rcarry_mul.
  ...
  Definition rsub
    := BoundsPipeline
         (sub_gen
            @ GallinaReify.Reify (Qnum limbwidth) @ GallinaReify.Reify (Z.pos (Qden limbwidth)) @ GallinaReify.Reify s @ GallinaReify.Reify c @ GallinaReify.Reify n @ GallinaReify.Reify (length c) @ GallinaReify.Reify coef)
         (Some tight_bounds, Some tight_bounds)
         (Some loose_bounds).
    ...
    := let str_n := n in
       ...
       | Some s, Some c
         => let tight_bounds := Some (tight_bounds n s c) in
            ...
            let ToFunLines s d name E arg_bounds
                := (name,
                    match E with
                    | Pipeline.Success E
                      => let E := @ToString.C.ToFunctionLines
                                    name s d E None arg_bounds in
                         match E with
            ...
            inl
              [("check_args" ++ NewLine ++ ToString.C.join NewLine header,
                check_args
                 ...
                 (ToFunLines _ _ "fecarry_mul" (rcarry_mul n s c machine_wordsize) (loose_bounds, loose_bounds));
                 (ToFunLines _ _ "fecarry" (rcarry n s c machine_wordsize) loose_bounds);
                 (ToFunLines _ _ "feadd" (radd n s c machine_wordsize) (tight_bounds, tight_bounds));
                 (ToFunLines _ _ "fesub" (rsub n s c machine_wordsize) (tight_bounds, tight_bounds));
                 (ToFunLines _ _ "feopp" (ropp n s c machine_wordsize) tight_bounds)]
       end.
