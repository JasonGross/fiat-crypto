...
Module base.
  Module type.
    Inductive type := Z | prod (A B : type).
  End type.
  Notation type := type.type.
End base.
...
Module parametric.
  ...
  Module base.
    Module type.
      Inductive type := Z | prod (A B : type) | var_with_subst (subst : btype).
...
Module ident.
  ...
  Section with_base.
    ...
    | Literal (v : Z) : pident parametric.base.type.Z
    | Add : pident (#base.type.Z -> #base.type.Z -> #base.type.Z)%ptype
    | Sub : pident (#base.type.Z -> #base.type.Z -> #base.type.Z)%ptype
    | Opp : pident (#base.type.Z -> #base.type.Z)%ptype
    ...
    Section with_var.
      ...
      Fixpoint structuredT {t} (p : pattern t) (codomain : Type) : Type
        := match p with
           ...
           | Any t idx => @expr base_type ident var t -> codomain
           | AnyLiteral t idx => type.interp base_interp t -> codomain
           | App s d f x => @structuredT _ f (@structuredT _ x codomain)
           end.

      Definition app_structured
    End with_var.
  End Rule.
...
Module partial.
  ...
  Section with_var.
    ...
    Context (abstract_domain' : base_type -> Type)
            (annotate : forall t, abstract_domain' t -> @expr var t -> @expr var t)
            (bottom' : forall A, abstract_domain' A)
            (base_reflect_lets : forall t : base_type, @expr var t -> UnderLets (@expr var t)).

    Definition value (t : type)
      := type.interpM
           UnderLets
           (fun t => abstract_domain' t * @expr var t)%type
           t.
    ...
    Fixpoint reify (is_let_bound : bool) {t} : value t -> type.for_each_lhs_of_arrow abstract_domain t -> UnderLets (@expr var t)
      := match t return value t -> type.for_each_lhs_of_arrow abstract_domain t -> UnderLets (@expr var t) with
         | type.base t
           => fun '(st, e) 'tt
              => let e' := annotate _ st e in
                 if is_let_bound
                 then base_reflect_lets _ e'
                 else Base e'
         | type.arrow s d
           => fun f '(sv, dv)
              => Base
    ...
    Fixpoint interp {t} (e : @expr value_with_lets t) : value_with_lets t
      := match e in expr.expr t return value_with_lets t with
         ...
         | expr.LetIn (type.base A) B x f
           => (x' <-- @interp _ x;
                 x'v <-- reify true x' tt;
                 @interp _ (f (Base (reflect x'v (state_of_value x')))))
         end%under_lets.
  ...
  Module wident.
    Section with_var.
      ...
      Context (inline_literals : bool)
              ...
              (update_literal_with_state : abstract_domain' base.type.Z -> Z -> Z)
              ...
              (ident_Literal : Z -> pident parametric.base.type.Z)
              (ident_Pair : forall A B, pident (#A -> #B -> #A * #B)%ptype)
              (ident_Fst : forall A B, pident (#A * #B -> #A)%ptype)
              (ident_Snd : forall A B, pident (#A * #B -> #B)%ptype)
              (invert_Literal : @expr var base.type.Z -> option Z)
              (invert_Pair : forall A B, @expr var (A * B) -> option (@expr var A * @expr var B)).

      Fixpoint base_reflect_lets {t : base.type} : @expr var t -> UnderLets (@expr var t)
        := match t return expr t -> UnderLets (expr t) with
           | base.type.Z
             => fun e
                => match invert_Literal e, inline_literals with
                   | Some v, true => Base e (* inline literals *)
                   | _, _ => UnderLet e (fun v => Base ($v)%expr)
                   end
           | base.type.prod A B
             => fun e
                => match invert_Pair _ _ e with
                   | Some (a, b) (* split up pairs *)
                     => (a' <-- @base_reflect_lets A a;
                           b' <-- @base_reflect_lets B b;
                           Base (#(ident_Pair A B) @ a' @ b')%expr)
                   | None => UnderLet e (fun v => Base ($v)%expr)
                   end%under_lets
           end.
    ...
    Context (interp_ident : forall t, ident t -> value_with_lets t).
      ...
      Fixpoint intersect_state_base_value {t} : abstract_domain' t -> base_value t -> base_value t
        := match t return abstract_domain' t -> base_value t -> base_value t with
           | base.type.Z => update_literal_with_state
           | base.type.prod _ _
             => fun st '(a, b)
...
Section specialized.
  ...
  Axiom evil : Z -> Z.
  Definition update_literal_with_state : abstract_domain' base.type.Z -> Z -> Z
    := fun upperbound n
       => match upperbound with
        | Some upperbound'
          => if Z.leb n upperbound' then n else evil n
        | None => n
        end.
