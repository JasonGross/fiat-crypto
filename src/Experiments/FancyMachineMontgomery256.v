...
Inductive type := TZ | Tbool | Prod : type -> type -> type.
...
Inductive trinop : type -> type -> type -> type -> Type :=
| OPZadc : trinop TZ TZ Tbool (Prod Tbool TZ)
| OPZsubc : trinop TZ TZ Tbool (Prod Tbool TZ)
| OPZsubc : trinop TZ TZ Tbool (Prod Tbool TZ)
...
Section exp.
  Context (modulus : Z) (m' : Z) (Hm : modulus <> 0) (H : 0 <= modulus < 2^256) (Hm' : 0 <= m' < 2^256) (ops : fancy_machine.instructions (2 * 128)) (props : fancy_machine.arithmetic ops).
  ...
  Definition partial_reduce_asm (v : fancy_machine.W * fancy_machine.W) : fancy_machine.W.
  Proof.
    cbv [proj1_sig partial_reduce] in f.
    pose (f v) as f'; subst f; cbv beta in f'.
    simpl @Z.mul in *.
    subst ldi.
    simpl in f'.
...
Section exp.
  ...
  Proof.
    ...
    simpl @Z.mul in *.
    subst ldi.
    set (RegPInv := (@Interface.ldi
                            (@ZBounded.SmallT (2 ^ 256) (2 ^ 256) modulus
                               (@ZLikeOps_of_ArchitectureBoundedOps 128 ops modulus 256))
                            (@fancy_machine.ldi (2 * 128) ops) m')) in *.
    repeat let cs := fresh in
           match eval cbv delta [f'] in f' with
           | appcontext[@ZBounded.ConditionalSubtract ?A ?B ?C ?D]
             => set (cs := @ZBounded.ConditionalSubtract A B C D) in (value of f');
                  cbv [ZBounded.ConditionalSubtract ZLikeOps_of_ArchitectureBoundedOps] in cs;
                  subst cs
           | appcontext[@ZBounded.DivBy_SmallBound ?A ?B ?C ?D]
             => set (cs := @ZBounded.DivBy_SmallBound A B C D) in (value of f');
                  cbv [ZBounded.DivBy_SmallBound ZLikeOps_of_ArchitectureBoundedOps] in cs;
                  subst cs
           | appcontext[@ZBounded.CarryAdd ?A ?B ?C ?D]
             => set (cs := @ZBounded.CarryAdd A B C D) in (value of f');
                  cbv [ZBounded.CarryAdd DoubleBounded.ripple_carry_adc DoubleBounded.ripple_carry_tuple DoubleBounded.ripple_carry_tuple' ZLikeOps_of_ArchitectureBoundedOps] in cs;
                  subst cs
           | appcontext[@ZBounded.Mod_SmallBound ?A ?B ?C ?D]
             => set (cs := @ZBounded.Mod_SmallBound A B C D) in (value of f');
                  cbv [ZBounded.Mod_SmallBound ZLikeOps_of_ArchitectureBoundedOps] in cs;
                  subst cs
           | appcontext[@ZBounded.modulus_digits ?A ?B ?C ?D]
             => set (cs := @ZBounded.modulus_digits A B C D) in (value of f');
                  cbv [ZBounded.modulus_digits ZLikeOps_of_ArchitectureBoundedOps] in cs;
                  subst cs
           | appcontext[@ZBounded.Mul ?A ?B ?C ?D]
             => set (cs := @ZBounded.Mul A B C D) in (value of f');
                  cbv [ZBounded.Mul ZLikeOps_of_ArchitectureBoundedOps] in cs;
                  subst cs
           | appcontext G[@Interface.adc _ {| adc := ?adcv |}]
             => let G' := context G[adcv] in change G' in (value of f')
           | appcontext G[@Interface.muldw _ {| muldw := ?muldwv |}]
             => let G' := context G[muldwv] in change G' in (value of f')
           | appcontext G[fst (?x, ?y)]
             => let G' := context G[x] in change G' in (value of f')
           | appcontext G[snd (?x, ?y)]
             => let G' := context G[y] in change G' in (value of f')
           end;
             cbv beta in *.
    set (RegMod := fancy_machine.ldi modulus) in *.
    set (RegZero := fancy_machine.ldi 0) in *.
    Notation "'plet' x := y 'in' z" := (Let_In y (fun x => z)) (at level 200, z at level 200, format "'plet'  x  :=  y  'in' '//' z").
    Notation "'slet' ( x , y ) := z 'in' w" := (let '(x, y) := z in w) (at level 200, w at level 200, format "'slet'  ( x ,  y )  :=  z  'in' '//' w").
Class is_var {T} (x : T) := Build_is_var : True.
Hint Extern 0 (is_var ?x) => is_var x; exact I : typeclass_instances.
Class not_is_var {T} (x : T) := Build_not_is_var : True.
Hint Extern 0 (not_is_var ?x) => tryif is_var x then fail else exact I : typeclass_instances.
    Ltac cis_var x := constr:(_ : is_var x).
    Ltac cnot_is_var x := constr:(_ : not_is_var x).
    Ltac cidtac x := constr:(ltac:(idtac x; exact I)).
    Ltac linearize_pairs linearize term at_bottom :=
      lazymatch term with
      | true => at_bottom term
      | false => at_bottom term
      | fst' ?x => linearize_pairs linearize (fst x) at_bottom
      | snd' ?x => linearize_pairs linearize (snd x) at_bottom
      | fst (?x, ?y) => linearize_pairs linearize x at_bottom
      | snd (?x, ?y) => linearize_pairs linearize y at_bottom
      | (fst ?x, snd ?x)
        => linearize_pairs
             linearize x ltac:(fun x'
                               => linearize_pairs
                                    linearize (fst x') ltac:(fun xv'0
                                                             => linearize_pairs
                                                                  linearize (snd x') ltac:(fun xv'1 => at_bottom (xv'0, xv'1))))
      | fst ?x
        => linearize_pairs linearize x ltac:(fun x' => let x'0 := match x' with
                                                                  | (?x0, ?x1) => x0
                                                                  | _ => constr:(fst x')
                                                                  end in
                                                       at_bottom x'0)
      ...
      | (?x, ?y)
        => linearize_pairs
             linearize x ltac:(fun x'
                     => linearize_pairs
                          linearize y ltac:(fun y'
                                            => at_bottom (x', y')))
      | ?term => linearize term at_bottom
      end.

    Ltac subterm_elim term at_bottom :=
      let term := (eval cbv beta in term) in
      linearize_pairs
        ltac:(fun term at_bottom
              => let xv := fresh in
                 let value := fresh in
                 match eval cbv beta in term with
                 | context[snd ?x]
                   => let test := cnot_is_var x in
                      lazymatch term with
                      | context[fst x]
                        => lazymatch eval pattern (fst x), (snd x) in term with
                           | (fun _ _ => ?P) _ _ => constr:(I : I)
                           | ?P ?fx ?sx
                             => subterm_elim
                                  x ltac:(fun x'
                                          => constr:(Let_In
                                                       x'
                                                       (fun xv =>
                                                          let '(x0, x1) := (fst xv, snd xv) in ltac:(let ret := subterm_elim (P x0 x1) at_bottom in exact ret))))
                           end
                      end
                 | context[@Interface.shl ?W ?shlv ?x ?c]
                   => lazymatch term with
                      | context[@Interface.shr W ?shrv x ?c']
                        => lazymatch eval pattern (@Interface.shl W shlv x c), (@Interface.shr W shrv x c') in term with
                           | (fun _ _ => ?P) _ _ => constr:(I : I)
                           | ?P ?fx ?sx
                             => subterm_elim
                                  x ltac:(fun x'
                                          => constr:(Let_In
                                                       x'
                                                       (fun xv =>
                                                          match xv with value => ltac:(let ret := subterm_elim (P (@Interface.shl W shlv value c) (@Interface.shr W shrv value c')) at_bottom in exact ret) end)))
                           end
                      end
                 | Let_In ?x ?y
                   => subterm_elim
                        x ltac:(fun x'
                                => constr:(Let_In
                                             x'
                                             (fun xv =>
                                                match xv with value => ltac:(let ret := subterm_elim (y value) at_bottom in exact ret) end)))
                 | let x := ?xv in @?y x
                   => subterm_elim (Let_In xv y) at_bottom
                 | let '(x, y) := eta ?v in @?b x y
                   => subterm_elim
                        v ltac:(fun v'
                                => constr:(let '(x, y) := eta v' in
                                           ltac:(let ret := subterm_elim (b x y) at_bottom in exact ret)))
                 | ?term' => at_bottom term'
                 end)
               term at_bottom.
    let f := (eval cbv delta [f'] in f') in
    let t := subterm_elim f ltac:(fun v => constr:(v)) in
    unify f t;
      clear f'; pose t as f'.
    Ltac linearize term at_bottom :=
      linearize_pairs
        ltac:(fun term at_bottom
              => let value := fresh in
                 let v' := fresh in
                        ...
                        x ltac:(fun x'
                                => linearize
                                     y ltac:(fun y'
                                             => linearize
                                                  b ltac:(fun b'
                                                          => constr:(let '(carry, v') := eta (fancy_machine.subc x' y' b')
                                                                     in ltac:(let ret := at_bottom (carry, v') in exact ret)))))
                 | @Interface.muldw _ ?muldw ?x ?y
                   => linearize
                        x ltac:(fun x'
                                => linearize
                                     y ltac:(fun y'
                                             => constr:(Let_In (@Interface.muldw _ muldw x' y')
                                                               (fun value => match value with v' => ltac:(let ret := at_bottom v' in exact ret) end))))
                 | Interface.adc ?x ?y ?b
                   => linearize
                        x ltac:(fun x'
                                => linearize
                                     y ltac:(fun y'
                                             => linearize
                                                  b ltac:(fun b'
                                                          => constr:(let '(carry, v') := eta (fancy_machine.adc x' y' b')
                                                                     in ltac:(let ret := at_bottom (carry, v') in exact ret)))))
                 | Interface.selc ?b ?x ?y
                   => linearize
                        x ltac:(fun x'
                                => linearize
                                     y ltac:(fun y'
                                             => linearize
                                                  b ltac:(fun b'
                                                          => constr:(Let_In (fancy_machine.selc b' x' y')
                                                                            (fun value => match value with v' => ltac:(let ret := at_bottom v' in exact ret) end)))))
                 | Let_In ?x ?b
                   => linearize
                        x ltac:(fun x'
                                => let bx := (eval cbv beta in (b x')) in
                                   linearize
                                     bx at_bottom)
                 | let '(x, y) := ?xv in @?b x y
                   => linearize
                        xv ltac:(fun xv'
                                 => linearize
                                      (fst xv') ltac:(fun xv'0
                                                      => linearize
                                                           (snd xv') ltac:(fun xv'1
                                                                           => let bx := (eval cbv beta in (b xv'0 xv'1)) in
                                                                              linearize
                                                                                bx at_bottom)))
                 | ?term' => match term' with
                             | _ => let test := cis_var term' in at_bottom term'
                             | _ => constr:(Let_In term' (fun value => match value with v' => ltac:(let ret := at_bottom v' in exact ret) end))
                             end
                 end)
               term at_bottom.
    let f := (eval cbv delta [f'] in f') in
    let t := linearize f ltac:(fun v => constr:(v)) in
    unify t f;
      clear f'; pose t as f'.
    pose (DoubleBounded.mul_double_multiply (fst v) (snd v)) as Mul256x256v.
    cbv delta [DoubleBounded.mul_double_multiply] in Mul256x256v.
    unfold Interface.muldw at 1 in (value of Mul256x256v).
    cbv beta delta [DoubleBounded.mul_double] in Mul256x256v.
    simpl @Interface.adc in Mul256x256v.
    let f := (eval cbv delta [Mul256x256v] in Mul256x256v) in
    let t := subterm_elim f ltac:(fun v => constr:(v)) in
    assert (f = t).
    ...
    { simpl.
      clear Mul256x256v; pose t as Mul256x256v.
    let f := (eval cbv delta [Mul256x256v] in Mul256x256v) in
    let t := subterm_elim f ltac:(fun v => constr:(v)) in
    unify f t;
      clear Mul256x256v; pose t as Mul256x256v.
...
linearize
                          y ltac:(fun y'
                                  => linearize
                                       b ltac:(fun b'
                                               => constr:(Let_In
                                                            (fancy_machine.subc x' y' b')
                                                            (fun v => ltac:(let ret := at_bottom v in exact ret))))))
      | ?term' =>
      end.
    let y' := linearize y in
           let b' := linearize b in
           constr:(Let_In x' (fun x0 => Let_In y' (fun y0 => Let_In (fancy_machine.subc x0 y0 b) (fun v => snd v)))) in (value of f')
      end.
      => let z' := match (eval pattern (snd (@ZBounded.CarryAdd A B C D x y)) in z) with ?z' _ => z' end in
         pose (Let_In (@ZBounded.CarryAdd A B C D x y)
                      (fun cv
                       => Let_In
                       => let '(carry, out) := (fst cv, snd cv) in
  ...
  Context (v : ZBounded.LargeT).
  pose  as f.
  cbv [proj1_sig partial_reduce] in f.
  simpl in f.
  Local Arguments
  cbv [proj1_sig partial_reduce LargeT ops' ZLikeOps_of_ArchitectureBoundedOps tuple tuple' ConditionalSubtract DivBy_SmallBound CarryAdd Mod_SmallBound modulus_digits Mul sprl_from_shift mul_double_multiply_low_low ripple_carry_adc fst snd mul_double sprl spread_left_from_shift] in f.
