...
Section language.
  ...
  Section with_var.
    ...
    Local Ltac invert_inline_directive' i :=
      ...
      | inline_directive Unit
        => revert dependent i;
           lazymatch goal with
           | [ |- forall i, @?G i ]
             => intro i;
                refine (match i in inline_directive t' return match t' return inline_directive t' -> _ with
                                                              | Unit => G
                                                              | _ => fun _ => True
                                                              end i
                        with
      ...
      | inline_directive (Prod ?A ?B)
        => is_var A; is_var B;
           move i at top;
           revert dependent A; intros A i;
           move i at top;
           revert dependent B; revert A;
           lazymatch goal with
           | [ |- forall A B i, @?G A B i ]
             => intros A B i;
                refine (match i in inline_directive t' return match t' return inline_directive t' -> _ with
                                                              | Prod A B => G A B
                                                              | _ => fun _ => True
    ...
    Local Ltac invert_exprf' i :=
      ...
      | exprf Unit
        => revert dependent i;
           ...
           | [ |- forall i, @?G i ]
             => intro i;
                refine (match i in Syntax.exprf _ _ t' return match t' return exprf t' -> _ with
                                                              | Unit => G
                                                              | _ => fun _ => True
                                                              end i
                        with
      ...
      | exprf (Tbase ?b)
        => move i at top; revert dependent b;
           lazymatch goal with
           | [ |- forall b i, @?G b i ]
             => intros b i;
                refine (match i in Syntax.exprf _ _ t' return match t' return exprf t' -> _ with
      ...
      | exprf (Prod ?A ?B)
        => move i at top; revert dependent A; intros A i;
           move i at top; revert dependent B; revert A;
           lazymatch goal with
           | [ |- forall A B i, @?G A B i ]
             => intros A B i;
                refine (match i in Syntax.exprf _ _ t' return match t' return exprf t' -> _ with
    ...
    Local Ltac invert_exprf :=
      match goal with
      | [ i : exprf _ |- _ ] => invert_exprf' i
      end.
    ...
    Proof.
      ...
      { constructor; auto.
        ...
        match goal with H : _ |- _ => apply H end.
        intros; destruct_head or; t_fin. }
      3:solve [ constructor; auto;
                intros;
                match goal with H : _ |- _ => apply H end;
                intros; destruct_head or; t_fin ].
      2:solve [match goal with H : _ |- _ => apply H end;
               intros; destruct_head or; t_fin ].
      { constructor; auto.
        intros.
        match goal with H : _ |- _ => apply H end.
