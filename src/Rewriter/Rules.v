          ...
          [mymap dont_do_again []
           ...
           ; mymap
               ...
               [ (* [do_again], so that if one of the arguments is concrete, we automatically get the rewrite rule for [Z_cast] applying to it *)
                 ...
                 (forall u1 u2 s x y,
                     0 <= singlewidth_upperbound -> 0 <= u1 -> 0 <= u2 -> ((u1+1) | (singlewidth_upperbound+1)) -> ((u2+1) | (singlewidth_upperbound+1)) -> (u1 <> singlewidth_upperbound \/ u2 <> singlewidth_upperbound)
                     -> cstZZ r[0~>u1] r[0~>u2] (Z.mul_split s x y)
                        = cstZZ r[0~>u1] r[0~>u2] (pairsinglewidth (Z.mul_split s x y)))
               ]
          ]%Z%zrange.
