...
Section __.
  ...
  Proof using Type.
    cbv [tight_bounds loose_bounds list_Z_bounded_by tight_upperbounds loose_upperbounds].
    intro.
    rewrite !fold_andb_map_map1, !fold_andb_map_iff; cbn [upper lower].
    setoid_rewrite Bool.andb_true_iff.
    intro H.
    autorewrite with distr_length natsimplify in *.
    setoid_rewrite In_nth_error_iff in H; setoid_rewrite In_nth_error_iff.
    repeat first [ lazymatch type of H with
                   | and _ _ => let H' := fresh in destruct H as [H' H]; split; [ assumption | ]
                   | forall v : _ * _, @ex nat _ -> _
                     => let x := fresh in let n := fresh "n" in intros [ [? ?] ? ] [n x]; revert x; specialize (fun v y => H v (ex_intro _ n y))
                   | forall v, Some _ = Some v -> _ => specialize (H _ eq_refl)
                   end
                 | match goal with
                   | [ H : ?x = ?y |- _ ]
